...
class App(Tk):
    def __init__(self):
        ...
        self.boton4 = Button(self, text='Eliminar documento',font=('Arial',12), command=lambda:self.eliminarDocumento(self.entry1.get()))
    ...
    def cambiarValor(self):
        self.label2.config(text=(f"Base de datos documental con {len(bdd.colecciones)} colecciones"))

    def borrarEntry1(self):
        self.entry1.delete(0,END)

    def obtener(self, nombre):
        self.label2.config(text=bdd.colecciones.get(nombre, None))

    def campoParaImportar(self):
        self.entryImportar = Entry(self, font=('Arial',14))
        self.entryImportar.place(x=400, y=50)
        self.botondeImportar = Button(self, text='importar',font=('Arial',12), command=lambda:[c.importarColeccion(self.entryImportar.get()), self.textodeImportar()])
    ...
    def obtenerDocumentoTexto(self):
        try:
            ...
            self.labelDocumentoNombre.config(text=(c.documentos[numero].contenido['Nombre']))
            self.labelDocumentoApellido.config(text=(c.documentos[numero].contenido['Apellido']))
            self.labelDocumentoEdad.config(text=(c.documentos[numero].contenido['Edad']))
            self.labelDocumentoEmail.config(text=(c.documentos[numero].contenido['Email']))
            self.labelDocumentoTelefono.config(text=(c.documentos[numero].contenido['Telefono']))
        except KeyError:
            self.limpiarDocumento()
            self.label3.config(text='El documento con id: {} no existe'.format(numero))

    def eliminarDocumento(self, numero):
        self.limpiarDocumento()
        numero = int(self.entry1.get())
        c.deleteDocumento(numero)
...
bdd = BaseDeDatos()
c = Coleccion(app.entry1.get())

app.mainloop()

##C:\\Users\\facun\\Escritorio\\datos_personales.csv